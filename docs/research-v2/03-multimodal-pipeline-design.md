...
> 调研日期：2026-04-09 | 定位：跨格式架构参考（具体选型以 [10-rag-selection-adr](./10-rag-selection-adr.md) 为准）
> 聚焦：五层 Pipeline 各层方案选型 — 解析 → 分类 → 模态处理 → 关系建模 → Chunk
> 相关文档：PDF 工具选型 [01](./01-parsing-tools-selection.md) | DOCX 解析 [02](./02-docx-parsing-research.md) | 分块 [04](./04-chinese-chunking-strategy.md)
